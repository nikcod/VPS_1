...
namespace Lean.Touch
{
	...
	public class xyzDragTranslate : MonoBehaviour
	{
		...
		private void Translate(Vector2 screenDelta)
		{
			...
			if (camera != null)
			{
				...
				// Add the deltaPosition
				if (x.enabled)
					transform.position += new Vector3(screenDelta.x * Sensitivity, 0, 0);
				if (y.enabled)
					transform.position += new Vector3(0, screenDelta.x * Sensitivity, 0);
				if (z.enabled)
					transform.position += new Vector3(0, 0, screenDelta.x * Sensitivity);
